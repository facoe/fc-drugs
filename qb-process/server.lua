...
RegisterNetEvent('qb-process:Server:CraftItem', function(item)
    local src = source
    local hasItems = 0
    local allItems = 0
    local inventory = {}
    ...
    if hasItems == allItems then
        ...
        TriggerClientEvent('qb-process:Notify', src, Config.Lang['item_collect']..tostring(item.name), Config.LangType['success'])
    else
        TriggerClientEvent('qb-process:Notify', src, Config.Lang['no_item'], Config.LangType['error'])
    end
end)
